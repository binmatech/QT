  ...
  if (loading) {
    return (
      <section className="relative pt-32 pb-20 px-6 md:px-12 overflow-hidden bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-12 items-center relative z-10 animate-pulse">
          <div className="lg:col-span-7 max-w-3xl">
            <div className="h-6 w-32 bg-slate-100 mb-6" />
            <div className="h-24 w-full bg-slate-100 mb-8" />
            <div className="h-6 w-96 bg-slate-100 mb-10" />
            <div className="flex gap-4">
              <div className="h-14 w-48 bg-slate-100" />
              <div className="h-14 w-48 bg-slate-100" />
            </div>
          ...
          <div className="lg:col-span-5 space-y-8">
            <div className="h-6 w-32 bg-slate-100" />
            {[1, 2, 3].map(i => <div key={i} className="h-24 bg-slate-100" />)}
          </div>
        </div>
      </section>
    );
        ...
        <div className="lg:col-span-7 max-w-3xl">
          ...
          >
            <div className="flex items-center gap-3 mb-6 text-[11px] font-bold uppercase tracking-widest text-brand-accent">
              <span className="bg-blue-50 px-2 py-0.5 rounded">{featured?.category}</span>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-slate-100 rounded-full overflow-hidden border border-slate-200">
                      <img 
                        src={`https://ui-avatars.com/api/?name=${featured.author}&background=random`} 
                        alt="Author" 
                        referrerPolicy="no-referrer"
                      />
                    </div>
                    <div>
                      <p className="font-serif font-bold text-lg leading-tight">{featured.author}</p>
                      <p className="text-xs text-slate-500">{featured.authorDesignation || 'Senior Editor'}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          >
            ...
            >
               ...
               <h4 className="text-2xl font-bold mt-2 mb-6">
                 {nextEvent ? nextEvent.title : "Events & Summits"}
               </h4>
               <div className="flex items-center justify-between">
                 <div className="flex flex-col">
                   <span className="text-xl font-mono font-bold">
                     {nextEvent ? nextEvent.date : "TBA"}
                   </span>
                   <span className="text-[10px] uppercase tracking-tighter opacity-60">
                     {nextEvent ? nextEvent.location : "Africa & World"}
                   </span>
                 </div>
                 <button className="bg-white text-brand-accent px-5 py-2 text-[10px] font-bold uppercase tracking-tighter rounded shadow-lg hover:bg-slate-50 transition-all">
                   {nextEvent?.registrationLink ? "Register" : "View Events"}
                 </button>
               </div>
            </div>
          </motion.div>
